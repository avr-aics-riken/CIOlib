...
// #################################################################
// インターバルステップの登録
void cio_DFI::setIntervalStep(int interval_step,
                              int base_step,
                              int start_step,
                              int last_step)
{
  //インターバルモードの登録（ステップ）
  m_intervalMngr.setMode(cio_Interval_Mngr::By_step);

  //インターバルステップの登録
  m_intervalMngr.setInterval(interval_step);

  //インターバル開始ステップの登録
  m_intervalMngr.setStart(start_step);

  //インターバル最終ステップの登録
  m_intervalMngr.setLast(last_step);

  //トリガーの初期化
  m_intervalMngr.initTrigger(base_step,0.0,0.0);
}

// #################################################################
// インターバルタイムの登録
void cio_DFI::setIntervalTime(double interval_time,
                              double dt,
                              double base_time,
                              double start_time,
                              double last_time)
{
  //インターバルモードの登録（タイム）
  m_intervalMngr.setMode(cio_Interval_Mngr::By_time);

  //インターバルタイムの登録
  m_intervalMngr.setInterval(interval_time);

  //インターバル開始タイムの登録
  m_intervalMngr.setStart(start_time);

  //インターバル終了タイムの登録
  m_intervalMngr.setLast(last_time);

  //トリガーの初期化
  m_intervalMngr.initTrigger(0,base_time,dt);
}

// #################################################################
// インターバルの計算に使われる全ての時間をスケールで無次元化する
bool cio_DFI::normalizeTime(const double scale)
{
  return m_intervalMngr.normalizeTime(scale);
}

// #################################################################
// インターバルのbase_timeをスケールで無次元化する
void cio_DFI::normalizeBaseTime(const double scale)
{
  m_intervalMngr.normalizeBaseTime(scale);
}

// #################################################################
// インターバルのintervalをスケールで無次元化する
void cio_DFI::normalizeIntervalTime(const double scale)
{
  m_intervalMngr.normalizeIntervalTime(scale);
}

// #################################################################
// インターバルのstart_timeをスケールで無次元化する
void cio_DFI::normalizeStartTime(const double scale)
{
  m_intervalMngr.normalizeStartTime(scale);
}

// #################################################################
// インターバルのlast_timeをスケールで無次元化する
void cio_DFI::normalizeLastTime(const double scale)
{
  m_intervalMngr.normalizeLastTime(scale);
}

// #################################################################
// インターバルのDetlaTをスケールで無次元化する
void cio_DFI::normalizeDelteT(const double scale)
{
  m_intervalMngr.normalizeDelteT(scale);
}
